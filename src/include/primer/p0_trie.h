...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept {
    ...
    for (auto i = other_trie_node.children_.begin(); i != other_trie_node.children_.end(); ++i) {
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if (child->key_char_ != key_char || children_.find(key_char) != children_.end()) {
      return nullptr;
    }
    auto i = children_.insert(std::pair<char, std::unique_ptr<TrieNode>>{key_char, std::move(child)});
  ...
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) {
    auto p = children_.find(key_char);
    return p == children_.end()? nullptr : &p->second;
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    latch_.WLock();
    std::unique_ptr<TrieNode> *now_ptr = &root_;
    for (std::basic_string<char>::size_type i = 0; i != key.length(); ++i) {
      auto now_key_char = key[i];
      std::unique_ptr<TrieNode>* child = (*now_ptr)->GetChildNode(now_key_char);

      if (child == nullptr) {
        if (i == key.length() - 1) {
          // If TrieNode with this ending character does not exist, create new TrieNodeWithValue
          now_ptr = (*now_ptr)->InsertChildNode(now_key_char,
                                                std::unique_ptr<TrieNode>(new TrieNodeWithValue<T>(now_key_char, value)));
        } else {
          now_ptr = (*now_ptr)->InsertChildNode(now_key_char,
                                                std::make_unique<TrieNode>(now_key_char));
        }
      } else {
        if (i == key.length() - 1 && !(*child)->IsEndNode()) {
          // If the terminal node is a TrieNode, then convert it into TrieNodeWithValue
          child->reset(new TrieNodeWithValue<T>(std::move(**child), value));
        } else if (i == key.length() - 1 && (*child)->IsEndNode()) {
          // If it is already a TrieNodeWithValue insertion fails and returns false
          latch_.WUnlock();
          return false;
        }
        now_ptr = child;
  ...
  bool Remove(const std::string &key) {
    ...
    for (std::basic_string<char>::size_type i = 0; i != key.length() ; ++i) {
      auto now_key_char = key[i];
      now_ptr = (*now_ptr)->GetChildNode(now_key_char);
      // key does not exist
      if (now_ptr == nullptr) {
        latch_.WUnlock();
        return false;
      ...
      v.push_back(now_ptr);
      // Toggle the is_end_ flag of terminal node to false
      if (i == key.length() - 1) (*now_ptr)->SetEndNode(false);
    }
    for (auto i = v.size() - 1; 0 < i; --i) {
      std::unique_ptr<TrieNode>* child = v[i];
      std::unique_ptr<TrieNode>* parent = v[i-1];
      // remove nodes that have no children and are not terminal node
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    std::unique_ptr<TrieNode> *now_ptr = &root_;
    for (std::basic_string<char>::size_type i = 0; i < key.length(); ++i) {
      now_ptr = (*now_ptr)->GetChildNode(key[i]);
      // If key does not exist in trie, set success to false.
      if (now_ptr == nullptr) {
        latch_.RUnlock();
        return {};
    ...
    auto p = dynamic_cast<TrieNodeWithValue<T>*>(now_ptr->get());
    if (p != nullptr) {
      *success = true;
      auto i = p->GetValue();
      latch_.RUnlock();
