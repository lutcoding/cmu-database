...
auto BufferPoolManagerInstance::NewPgImp(page_id_t *page_id) -> Page * {
  ...
  frame_id_t available_frame_id;
  if (!free_list_.empty()) {
    available_frame_id = free_list_.front();
    free_list_.pop_front();
  } else {
    bool ok = replacer_->Evict(&available_frame_id);
    if (!ok) {
      return nullptr;
    }
    if (pages_[available_frame_id].IsDirty()) {
      disk_manager_->WritePage(pages_[available_frame_id].page_id_, pages_[available_frame_id].data_);
    }
